...
* create a yaml configuration file ```touch config.yaml``` in the root directory & add the following content.
...
Now that you have a da node and light client running, we are ready to build and run our Cosmos-SDK blockchain (here we have considered [gm application](https://rollkit.dev/tutorials/gm-world))
...
* start your rollup
    ...
    create a script file (init-local.sh) in root folder
